# Documentation Index

## Governance and Working Method
- `ENGINEERING_MANIFEST.md`
- `WORKING_AGREEMENT.md`
- `HANDOFF_MANIFEST.md`
- `HANDOFF_GUIDE.md`
- `RELEASE_PROCESS.md`
- `TEST_STRATEGY.md`
- `OBSERVABILITY.md`
- `QUICKSTART.md`
- `RUNBOOK.md`
- `PROJECT_STATE.md`

## Concepts
- `concepts/NEW_NFL_SYSTEM_CONCEPT_v0_1.md`
- `concepts/NEW_NFL_SYSTEM_CONCEPT_v0_2.md`
- `concepts/NEW_NFL_PHASE1_SCOPE_v0_1.md`
- `concepts/NEW_NFL_SOURCE_GOVERNANCE_v0_1.md`
- `concepts/NEW_NFL_METADATA_MODEL_v0_1.md`
- `concepts/NEW_NFL_PHYSICAL_PLATFORM_BLUEPRINT_v0_1.md`
- `concepts/NEW_NFL_METADATA_SCHEMA_OUTLINE_v0_1.md`
- `concepts/NEW_NFL_TECHNICAL_BOOTSTRAP_v0_1.md`

## Architecture Decisions
- `adr/ADR-0001-repo-and-operating-model.md`
- `adr/ADR-0002-data-platform-and-storage-decision-frame.md`
- `adr/ADR-0003-ingestion-layering.md`
- `adr/ADR-0004-web-application-strategy-frame.md`
- `adr/ADR-0005-scheduler-and-vps-runtime-model-frame.md`
- `adr/ADR-0006-phase1-scope-boundary.md`
- `adr/ADR-0007-source-tiering-and-fallback-policy.md`
- `adr/ADR-0008-provenance-and-audit-model.md`
- `adr/ADR-0009-entity-identity-and-canonical-keys.md`
- `adr/ADR-0010-physical-storage-and-directory-layout.md`
- `adr/ADR-0011-metadata-registry-and-audit-schema.md`
- `adr/ADR-0012-canonical-schema-family-boundaries.md`
- `adr/ADR-0013-python-runtime-and-toolchain.md`
- `adr/ADR-0014-project-layout-and-entrypoints.md`
- `adr/ADR-0015-quality-gates-and-bootstrap-scope.md`

## Handoffs
- `docs/_handoff/HANDOFF_A0_1_architecture_concept.md`
- `docs/_handoff/HANDOFF_A0_2_data_platform_and_scope.md`
- `docs/_handoff/HANDOFF_A0_3_source_governance_and_metadata.md`
- `docs/_handoff/HANDOFF_A0_4_physical_platform_blueprint.md`
- `docs/_handoff/HANDOFF_T1_0_technical_bootstrap.md`
