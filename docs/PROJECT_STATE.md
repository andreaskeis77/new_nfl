...
**T2.6A UI-Fundament** — abgeschlossen (2026-04-23). Nächster Bolt: T2.6B (Home/Freshness auf `mart.freshness_overview_v1`).
...
**T2.6B — Home / Freshness-Dashboard** gemäß `T2_3_PLAN.md` §5: `mart.freshness_overview_v1` neu (abgeleitet aus `meta.load_event` + `meta.quarantine_case` je Domäne mit `last_run_at`, `last_status`, `open_quarantine_count`), Home-View ersetzt den Skelett-Platzhalter in `home.html` durch echte Freshness-Kacheln pro Domäne (`<FreshnessBadge>`-Pills mit Status-Farbe und relativer Zeit) und Quarantäne-Zähler je Domäne. Erste echte UI-View, die ausschließlich aus `mart.*` liest und die Komponenten-Bibliothek aus T2.6A konsumiert. Risiken: `meta.*`-vs-`mart.*`-Abgrenzung (`meta.load_event` ist kein Mart, muss entweder via Mart-Projektion oder über eine dedizierte Freshness-Service-Funktion gelesen werden), Zeitzonen-Darstellung (UTC vs. lokal), Empty-Cold-Start-Fall (bevor irgendein Run existiert).
