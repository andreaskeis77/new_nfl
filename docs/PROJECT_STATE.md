...
**T2.5E Team Stats Aggregate** — abgeschlossen (2026-04-22). Nächster Bolt: T2.5F (Player Stats Aggregate).
...
**T2.5F — Player Stats Aggregate** gemäß `T2_3_PLAN.md` §4: `core.player_stats_weekly` aus nflverse (Tier-A) + `official_context_web` (Tier-B, Cross-Check auf ausgewählte Kernfelder), `mart.player_stats_season_v1` und `mart.player_stats_career_v1` als Read-Projektionen. Zweite aggregierende Domäne — die neuen Risiken sind Multi-Position-Spieler (Edge Case Taysom Hill), Joins zu `core.player`/`core.roster_membership` für Team-Kontext pro Woche und Karriere-Aggregate über mehrere Saisons. T2.5E-Pattern (Bye-Week-tolerante `COUNT(metric)`, Dedupe per `_loaded_at DESC`, Tier-B-Cross-Check auf Kernmetriken) gilt weiter; `CorePlayerStatsLoadResult` muss `CoreLoadResultLike` erfüllen.
