...
**T2.5F Player Stats Aggregate** — abgeschlossen (2026-04-22). Nächster Bolt: T2.6A (ADR-0030 Accepted + Tailwind-Setup + UI-Komponenten-Skelett).
...
- ADR-0025, ADR-0026, ADR-0027, ADR-0028, ADR-0029, ADR-0031 — `Accepted`; ADR-0030 (UI Stack) bleibt `Proposed` bis T2.6A; ADR-0032 (bitemporale Roster-Modellierung) `Proposed` seit T2.5D
...
**T2.6A — UI-Fundament** gemäß `T2_3_PLAN.md` §5: ADR-0030 (UI-Stack Jinja + Tailwind + htmx + Observable Plot) final `Accepted`, Tailwind-Build-Chain in `ui/` mit CSS-Pipeline, Komponenten-Skelett (Layout, Navigation, Tabellen-Partial, Detail-Panel) und erstes gemeinsames `base.html`-Template. Dies ist der erste Tranche der UI-Schicht — alle Read-Pfade lesen weiterhin ausschließlich aus `mart.*` (ADR-0029); der Lint-Test gegen `core.*`/`stg.*`/`raw/` bleibt aktiv und muss für alle neuen UI-Module gelten. Risiken: Windows-kompatible Node/Tailwind-Toolchain-Wahl, Artefakt-Pfad für gebauten CSS-Output (nicht committet vs. committet), htmx-Integration ohne globale JS-Dependencies.
