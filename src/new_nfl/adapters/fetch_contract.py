...
ContractRunResult = AdapterExecutionResult
...
def execute_adapter_contract(
    ...
    *,
    ...
    load_event_id = record_load_event(
        ...
        pipeline_name=pipeline_name,
        event_kind="landed_contract",
        object_path=str(receipt_path),
    )
...
def execute_fetch_contract(
    settings: Settings,
    *,
    adapter_id: str,
    execute: bool,
) -> ContractRunResult:
    return execute_adapter_contract(
        settings,
        adapter_id,
        execute=execute,
        triggered_by="cli",
